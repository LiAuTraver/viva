...
/// @param _Out_ means the function writes to the pointer.
/// @param _Inout_ means the function reads from and writes to the pointer.
/// @param _Ret_maybenull_ means the function may return nullptr.
/// @param _Ret_maybe_result(expr) means the function may return the result of the expression.
/// @param _Generic_usage means the function uses the _Generic keyword.
/// @param viva_param_nonnull(...) means the function requires the parameters to be non-null.
...
#define viva_param_nonnull(...) __attribute__((nonnull(__VA_ARGS__)))
...
static __attribute__((always_inline)) inline void *viva_alloc(const size_t _size) { return malloc(_size); }
struct viva_forward_list_header;
struct viva_forward_list_node;

static _Ret_maybenull_ _Generic_usage void *viva_cstd_impl_forward_list_empty_header();

static _Generic_usage size_t viva_cstd_impl_forward_list_size(_In_ void *header) viva_param_nonnull(1);

static _Generic_usage bool viva_cstd_impl_forward_list_is_empty(_In_ void *header);

static _Generic_usage viva_status_t viva_cstd_impl_forward_list_drop(_In_ void *header) viva_param_nonnull(1);

static _Generic_usage viva_status_t viva_cstd_impl_forward_list_pop_front(_Inout_ void *header);

static _Generic_usage void viva_cstd_impl_forward_list_clear(_Inout_ void *header) viva_param_nonnull(1);

static _Generic_usage void viva_cstd_impl_forward_list_free(_Inout_ void **const header_ptr) viva_param_nonnull(1);

static _Generic_usage void viva_cstd_impl_forward_list_reverse(_Inout_ void *header) viva_param_nonnull(1);

static _Ret_maybenull_ _Generic_usage struct viva_forward_list_node *viva_cstd_impl_forward_list_end(_In_ void *header)
	viva_param_nonnull(1);

static _Ret_maybenull_ _Generic_usage struct viva_forward_list_node *
viva_cstd_impl_forward_list_begin(_In_ void *header) viva_param_nonnull(1);

static viva_status_t viva_cstd_impl_forward_list_erase_if(_Inout_ void *header, _In_ bool (*predicate)(void *));

static _Ret_maybenull_ void *viva_cstd_impl_forward_list_find_first_if(_In_ void *header,
																																			 _In_ bool (*predicate)(void *))
	viva_param_nonnull(1, 2);
...
#define VIVA_CSTD_FORWARD_LIST_COMMON_MEMBER_()                                                                        \
	typeof(viva_cstd_impl_forward_list_empty_header) *const	 new_header;                                                 \
	typeof(viva_cstd_impl_forward_list_size) *const					 size;                                                       \
	typeof(viva_cstd_impl_forward_list_is_empty) *const			 is_empty;                                                   \
	typeof(viva_cstd_impl_forward_list_drop) *const					 drop;                                                       \
	typeof(viva_cstd_impl_forward_list_pop_front) *const		 pop_front;                                                  \
	typeof(viva_cstd_impl_forward_list_clear) *const				 clear;                                                      \
	typeof(viva_cstd_impl_forward_list_free) *const					 free;                                                       \
	typeof(viva_cstd_impl_forward_list_reverse) *const			 reverse;                                                    \
	typeof(viva_cstd_impl_forward_list_end) *const					 end;                                                        \
	typeof(viva_cstd_impl_forward_list_begin) *const				 begin;                                                      \
	typeof(viva_cstd_impl_forward_list_erase_if) *const			 erase_if;                                                   \
	typeof(viva_cstd_impl_forward_list_find_first_if) *const find_first_if;
//////////////////// End of The Macro ////////////////////

/////////////////// Begin of The Macro ///////////////////
#define VIVA_CSTD_FORWARD_LIST_COMMON_MEMBER_INIT()                                                                    \
	.new_header = viva_cstd_impl_forward_list_empty_header, .size = viva_cstd_impl_forward_list_size,                    \
	.is_empty = viva_cstd_impl_forward_list_is_empty, .drop = viva_cstd_impl_forward_list_drop,                          \
	.pop_front = viva_cstd_impl_forward_list_pop_front, .clear = viva_cstd_impl_forward_list_clear,                      \
	.free = viva_cstd_impl_forward_list_free, .reverse = viva_cstd_impl_forward_list_reverse,                            \
	.end = viva_cstd_impl_forward_list_end, .begin = viva_cstd_impl_forward_list_begin,                                  \
	.erase_if = viva_cstd_impl_forward_list_erase_if, .find_first_if = viva_cstd_impl_forward_list_find_first_if
//////////////////// End of The Macro ////////////////////

struct viva_forward_list {
	VIVA_CSTD_FORWARD_LIST_COMMON_MEMBER_()
} static const Forward_list = {VIVA_CSTD_FORWARD_LIST_COMMON_MEMBER_INIT()};
...
inline void viva_cstd_impl_forward_list_free(_Inout_ void **const header_ptr) {
	viva_cstd_impl_forward_list_clear(*header_ptr);
	free(*header_ptr);
	*header_ptr = nullptr;
}

inline _Generic_usage void viva_cstd_impl_forward_list_reverse(_Inout_ void *header) {
...
#define VIVA_CSTD_INSTANTIATE_FORWARD_LIST_OF_TYPE_IMPL(data_type_)                                                      \
	...
	static _Ret_maybe_result(struct viva_forward_list_node *) _Ret_notnull_ viva_result_t                                  \
											 viva_cstd_impl_forward_list_create_forward_list_node_of_##data_type_(_In_ const data_type_ data); \
	static viva_status_t viva_cstd_impl_forward_list_push_front_of_##data_type_(_Inout_ void	 *header,                    \
																																							_In_ data_type_ data);                     \
	static viva_status_t viva_cstd_impl_forward_list_push_back_of_##data_type_(_Inout_ void		*header,                     \
																																						 _In_ data_type_ data);                      \
	static viva_status_t viva_cstd_impl_forward_list_insert_after_of_##data_type_(_In_ void *header, _In_ void *node,      \
																																								_In_ data_type_ data);                   \
	static viva_status_t viva_cstd_impl_forward_list_unique_of_##data_type_(_Inout_ void *header);                         \
                                                                                                                         \
	struct {                                                                                                               \
		VIVA_CSTD_FORWARD_LIST_COMMON_MEMBER_()                                                                              \
		typeof(viva_cstd_impl_forward_list_create_forward_list_node_of_##data_type_) *const create_with;                     \
		typeof(viva_cstd_impl_forward_list_push_front_of_##data_type_) *const								push_front;                      \
		typeof(viva_cstd_impl_forward_list_push_back_of_##data_type_) *const								push_back;                       \
		typeof(viva_cstd_impl_forward_list_insert_after_of_##data_type_) *const							insert_after;                    \
		typeof(viva_cstd_impl_forward_list_unique_of_##data_type_) *const										unique;                          \
	...
	inline viva_result_t viva_cstd_impl_forward_list_create_forward_list_node_of_##data_type_(const data_type_ data) {     \
		...
		if (node == nullptr)                                                                                                 \
			VIVA_RETURN_ERROR(kBadAlloc, "Failed to allocate memory for forward_list_node")                                    \
		val header = viva_alloc(sizeof(struct viva_forward_list_header));                                                    \
		if (header == nullptr) {                                                                                             \
			free(node->data_ptr);                                                                                              \
			free(node);                                                                                                        \
			VIVA_RETURN_ERROR(kBadAlloc, "Failed to allocate memory for forward_list_header")                                  \
		}                                                                                                                    \
	...
	inline viva_status_t viva_cstd_impl_forward_list_insert_after_of_##data_type_(void *header, void *node,                \
																																								data_type_ data) {                       \
		if (header == nullptr)                                                                                               \
			return kEmpty;                                                                                                     \
		if (node == nullptr)                                                                                                 \
	...
	inline viva_status_t viva_cstd_impl_forward_list_unique_of_##data_type_(void *header) {                                \
		if (header == nullptr)                                                                                               \
			return kEmpty;                                                                                                     \
		for (var current = ((struct viva_forward_list_header *)header)->head; current && current->next;                      \
				 current		 = current->next) {                                                                                      \
	...
	inline struct viva_forward_list_node *viva_cstd_impl_forward_list_create_forward_list_node_impl_of_##data_type_(       \
		data_type_ data) {                                                                                                   \
...
#define instantiate_forward_list_of(data_type_) VIVA_CSTD_INSTANTIATE_FORWARD_LIST_OF_TYPE(data_type_)
#endif
