...
static inline struct VIVA_CURSOR_SINGLETON viva_cstd_impl_cursor_move_cursor_win(short, short);
...
static inline struct VIVA_CURSOR_SINGLETON viva_cstd_impl_cursor_putc(wchar_t);
static inline struct VIVA_CURSOR_SINGLETON viva_cstd_impl_cursor_set_cursor_win(COORD);
static inline struct VIVA_CURSOR_SINGLETON viva_cstd_impl_cursor_putc_at_coords(COORD, wchar_t);
...
inline static struct VIVA_CURSOR_SINGLETON viva_cstd_impl_cursor_move_cursor_win(const short row, const short col) {
	...
	coord.X += col;
	coord.Y += row;
	contract_assert(coord.X >= 0 && coord.Y >= 0 && (bool)("Invalid cursor position"));
